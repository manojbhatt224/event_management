...
class EventsController {
  ...
  static validateUpdateEvent(updateEventData) {
    ...
    if (
      "title" in updateEventData &&
      typeof updateEventData.title !== "string"
    ) {
      errors.title = "Title is required and must be a string";
    }
    if (
      "category" in updateEventData &&
      typeof updateEventData.category !== "string"
    ) {
      errors.category = "Category is required and must be a string";
    }
    if (
      "description" in updateEventData &&
      typeof updateEventData.description !== "string"
    ) {
      errors.description = "Description is required and must be a string";
    }

    if (
      "totalParticipants" in updateEventData &&
      (typeof updateEventData.totalParticipants !== "number" ||
        updateEventData.totalParticipants < 0)
    ) {
      errors.totalParticipants =
        "Total participants is required and must be a non-negative number";
    }

    if (
      "startDate" in updateEventData &&
      !EventsController.isValidDate(updateEventData.startDate)
    ) {
      errors.startDate =
        "Start date is required and must be a valid date in the format YYYY-MM-DD";
    }

    if (
      "endDate" in updateEventData &&
      !EventsController.isValidDate(updateEventData.endDate)
    ) {
      errors.endDate =
        "End date is required and must be a valid date in the format YYYY-MM-DD";
    }

    if (
      "location" in updateEventData &&
      typeof updateEventData.location !== "string"
    ) {
  ...
  static async addEvent(req, res) {
    ...
    const errors = EventsController.validateEvent(
      title,
      category,
      description,
      totalParticipants,
  ...
  static async filterEvents(req, res) {
    ...
    const { title, startDate, endDate } = filterData;

    if (title && startDate && endDate) {
      try {
        const results = await eventDAO.searchEventsByTitleAndDate(
          startDate,
          endDate,
          title
        );
        if (results) {
          res.sendData(200, { events: results });
        } else {
          res.sendData(200, { events: [] });
        }
      } catch (err) {
        res.sendData(401, { error: err });
      }
    } 
    else if(startDate && endDate){
      try {
        const results = await eventDAO.getEventByDateRange(startDate, endDate);
        if (results) {
          res.sendData(200, { events: results });
        } else {
      ...
      try {
        const results = await eventDAO.searchEventByTitle(title);
        if (results) {
          res.sendData(200, { events: results });
        } else {
          res.sendData(200, { events: [] });
      ...
      res.sendData(401, { error: "No valid filter inputs." });
    }
  }
}
